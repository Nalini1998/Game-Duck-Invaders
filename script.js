function preload() {
  this.load.image('bug1', 'https://img.icons8.com/color/40/000000/duck.png');
  this.load.image('bug2', 'https://img.icons8.com/color/48/000000/duck.png');
  this.load.image('bug3', 'https://img.icons8.com/color/48/000000/duck.png');
  this.load.image('platform', 'https://content.codecademy.com/courses/learn-phaser/physics/platform.png');
  this.load.image('codey', 'https://img.icons8.com/external-flaticons-lineal-color-flat-icons/64/external-cat-animal-flaticons-lineal-color-flat-icons-3.png');
...
function create() {
	...
	gameState.scoreText = this.add.text(175, 482, 'Duck Left: 24', { fontSize: '15px', fill: '#000000' });
  ...
  // we’ll use to create individual pellets
  ...
  const genPellet = () => {
    ...
    this.physics.add.collider(gameState.enemies, gameState.bugRepellent, (bug, repellent) => {
      bug.destroy();
      repellent.destroy();
      gameState.scoreText.setText(`Duck Left: ${numOfTotalEnemies()}`)
    });
   ...
   this.physics.add.collider(gameState.enemies, gameState.player, () => { 
      ...
      gameState.enemyVelocity = 1;
      this.physics.pause();
      this.add.text(210, 250, 'Game Over!!! Restart?', { fontSize: '12px', fill: '#000' });
    });    
...
function update() {
    ...
    if (numOfTotalEnemies() === 0) {
      ...
      this.physics.pause();
      gameState.enemyVelocity = 1;
      this.add.text(100, 250, 'You have won!!!', {fontSize: '12px', fill: '#000000'});
    } else {
